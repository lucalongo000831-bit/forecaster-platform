...
export async function getForecastAnalysis(symbolInput: string, horizon: ForecastHorizon = "1m", targetOverride?: number | null, stopOverride?: number | null, currencyOverride?: string | null) {
  ...
  const analysis = analyzeForecast({
    symbol, horizon, currency: currencyOverride ?? targets?.analysis.currency ?? "USD", bars: technical.analysis.input,
