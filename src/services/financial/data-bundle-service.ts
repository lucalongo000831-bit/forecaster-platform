...
export async function getAnalysisDataBundle(symbolInput: string): Promise<AnalysisDataBundle> {
  ...
  return (await providerCached(`analysis-bundle:${symbol}`, { freshSeconds: 3_600, staleSeconds: 21_600 }, async () => {
    ...
    const analystData = settled(analyst, "analystConsensus", ["fmp", "eodhd", "yahoo"]); const peersData = settled(peers, "peers", ["fmp", "finnhub", "yahoo"]); const insiderData = settled(insiders, "insiderTransactions", ["finnhub", "sec-edgar", "yahoo"]); const dividendData = settled(dividends, "dividends", ["fmp", "eodhd", "yahoo"]); const ownershipData = settled(ownership, "ownership", ["yahoo"]);
    ...
    if (insiderData) lineage.push({ field: "insiderTransactions", provider: insiderData.some((item) => "accessionNumber" in item) ? "sec-edgar" : insiderData.some((item) => "provider" in item && item.provider === "yahoo") ? "yahoo" : "finnhub", sourceTimestamp: insiderData[0]?.filingDate ?? null, fetchedAt: new Date().toISOString(), quality: insiderData.length ? "verified" : "partial" });
    if (ownershipData) lineage.push({ field: "ownership", provider: "yahoo", sourceTimestamp: ownershipData.institutions[0]?.reportDate ?? null, fetchedAt: new Date().toISOString(), quality: ownershipData.institutions.length ? "partial" : "unavailable" });
