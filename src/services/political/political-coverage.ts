...
import type { PoliticalHistoryMonthCoverage } from "@/types";
import { summarizePoliticalHistoryMonths } from "./political-history-coverage";
...
export interface PoliticalCoverageHealth {
  ...
  historyMonths?: PoliticalHistoryMonthCoverage[];
}
...
export function politicalCoverage(period: PoliticalPeriod, rows: PoliticalTransaction[], allRows: PoliticalTransaction[], health: PoliticalCoverageHealth, now = new Date()): PoliticalDatasetCoverage {
  ...
  const temporalCoverage = Math.min(100, observedDays / requiredDays * 100); const monthSummary = requestedFrom && health.historyMonths ? summarizePoliticalHistoryMonths(health.historyMonths, requestedFrom, requestedTo) : null; const historyCoveragePercent = monthSummary ? Math.min(temporalCoverage, monthSummary.coveragePercent) : temporalCoverage; const ingestedRecords = Math.max(health.totalRecords, allRows.length); const mappingRate = health.mappingRate || (allRows.length ? allRows.filter((row) => row.resolutionStatus === "RESOLVED").length / allRows.length * 100 : 0); const sourceHealthy = health.databaseStatus === "AVAILABLE" && (health.operationalStatus === "OK" || health.fmpStatus === "OK");
  ...
  else if (!sourceHealthy || historyCoveragePercent < 95 || mappingRate < 98 || (monthSummary !== null && !monthSummary.complete)) { status = "PARTIAL_DATA"; reason = `No matching record was found, but zero cannot be verified: history ${historyCoveragePercent.toFixed(0)}%, issuer mapping ${mappingRate.toFixed(1)}%.`; suggestedPeriod = period !== "1Y" && observedDays >= 365 ? "1Y" : null; }
