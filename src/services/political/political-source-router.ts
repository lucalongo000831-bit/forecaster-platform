...
export class PoliticalSourceRouter {
  ...
  async historicalPage(page: number, pageSize = 100, members?: Awaited<ReturnType<CapitolExposedCongressAdapter["getMembers"]>>) {
    const lookup = members ?? await this.historical.getMembers(); const response = await this.historical.getPage(page, pageSize); const rows = response.data.data.flatMap((row) => { const mapped = mapCapitolExposedTrade(row, row.member_id ? lookup.get(row.member_id) : undefined); return mapped ? [mapped] : []; }); return { rows, hasMore: response.data.meta.has_more, total: response.data.meta.total, provider: "capitol-exposed" as const, fetchedAt: response.fetchedAt };
  }

  getHistoricalMembers() { return this.historical.getMembers(); }
