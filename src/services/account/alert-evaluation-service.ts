  ...
  for (const record of records) {
    ...
    try {
      ...
      if (technicalEvaluation) {
        ...
        if (result.triggered && alert.triggeredAt && now.getTime() - alert.triggeredAt.getTime() < cooldownMinutes * 60_000) {
          cooldownSuppressed = true;
          result = { ...result, triggered: false, message: `${result.message} Cooldown active; this transition was recorded but will not be notified later.` };
        }
