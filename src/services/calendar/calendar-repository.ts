...
export async function loadPersistedCalendar(from: string, to: string, symbol?: string): Promise<MarketCalendarAnalysis | null> {
  ...
  try {
    ...
    const macroProviders = releases.length ? [...new Set(releases.map((row) => row.provider))].join(" / ") : null;
    const availability = Object.fromEntries(categories.map((type) => { const rows = uniqueEvents.filter((event) => event.type === type); const verifiedMacroCategory = (type === "MACRO" || type === "CENTRAL_BANK") && releases.length > 0; const available = rows.length > 0 || verifiedMacroCategory; return [type, { status: available ? "AVAILABLE" as const : "SOURCE_UNAVAILABLE" as const, provider: rows.length ? [...new Set(rows.map((event) => event.provider))].join(" / ") : verifiedMacroCategory ? macroProviders : null, reason: available ? null : "Nessun dataset persistito per questa categoria; zero non verificato.", count: available ? rows.length : null, lastUpdated: available ? calculatedAt : null, isLastKnownGood: true }]; })) as MarketCalendarAnalysis["availability"];
