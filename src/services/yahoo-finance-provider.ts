...
export class YahooFinanceProvider implements FinancialDataProvider {
  ...
  async getPoliticalActivity(ref: InstrumentRef) {
    ...
    return this.fallback<PoliticalData>("political", symbol, async () => {
      ...
      return {
        chartSeries,
        trades: disclosures.map((item) => ({ id: item.id, name: item.politician, role: item.chamber === "SENATE" ? "Senator" : "Representative", party: item.party ?? undefined, region: item.chamber, type: item.transactionType === "PURCHASE" ? "BUY" as const : item.transactionType.startsWith("SALE") ? "SELL" as const : item.transactionType === "EXCHANGE" ? "EXCHANGE" as const : "OTHER" as const, published: item.disclosureDate ?? "Dato non disponibile", traded: item.transactionDate, amount: item.amountRange ?? "Dato non disponibile", amountLevel: 1, asset: item.asset, ownership: item.ownership, disclosureDelayDays: delayDays(item.transactionDate, item.disclosureDate), capitalGains: item.capitalGains, sourceUrl: item.sourceUrl })),
        source: "fmp" as const,
