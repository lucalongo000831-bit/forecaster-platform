...
function creditMetrics(charts: Map<string, ChartRecord>): RiskMetric[] { const hyg = statsFor(charts, "HYG"); const lqd = statsFor(charts, "LQD"); const xlf = statsFor(charts, "XLF"); const relative = hyg?.oneMonth !== null && hyg?.oneMonth !== undefined && lqd?.oneMonth !== null && lqd?.oneMonth !== undefined ? hyg.oneMonth - lqd.oneMonth : null; return [
...
function liquidityMetrics(quotes: Map<string, MarketQuoteDto>, charts: Map<string, ChartRecord>): RiskMetric[] { const spy = statsFor(charts, "SPY"); const uup = statsFor(charts, "UUP"); const tlt = statsFor(charts, "TLT"); const xlf = statsFor(charts, "XLF"); const quote = quotes.get("SPY"); const spread = quote?.bid && quote.ask && quote.price ? (quote.ask - quote.bid) / quote.price * 10_000 : null; return [
  ...
  metric("financial_stress", "Financial sector drawdown", xlf?.drawdown52Week ?? null, downsideStress(xlf?.drawdown52Week, 25), { displayValue: fmt(xlf?.drawdown52Week ?? null, "%"), dataType: xlf?.drawdown52Week !== null && xlf?.drawdown52Week !== undefined ? "PROXY" : "UNAVAILABLE", source: "XLF proxy" }),
]; }

function ratesMetrics(quotes: Map<string, MarketQuoteDto>, macro: MacroObservation[][]): RiskMetric[] { const ten = quotes.get("^TNX"); const two = quotes.get("^UST2Y"); const groups = macro.flat(); const latest = (indicator: MacroObservation["indicator"]) => groups.filter((item) => item.indicator === indicator && item.value !== null).sort((a, b) => b.date.localeCompare(a.date))[0]; const fed = latest("RATES"); const inflation = latest("INFLATION"); const curve = ten?.price !== undefined && two?.price !== undefined ? ten.price - two.price : null; const curveState = curve === null ? "UNKNOWN" : curve < -0.1 ? "INVERTED" : curve < 0.2 ? "FLAT" : "NORMAL"; return [
  metric("fed_funds", "Fed funds rate", fed?.value ?? null, scale(fed?.value ?? null, 2, 7), { displayValue: fmt(fed?.value ?? null, "%"), dataType: fed ? "DIRECT" : "UNAVAILABLE", source: "FMP / Alpha Vantage", asOf: fed?.date ?? null }),
  metric("us_2y", "US 2Y yield", two?.price ?? null, scale(two?.price ?? null, 2, 7), { displayValue: fmt(two?.price ?? null, "%"), dataType: two ? "DIRECT" : "UNAVAILABLE", source: sourceName(two?.source), asOf: two?.asOf ?? null }),
  metric("us_10y", "US 10Y yield", ten?.price ?? null, scale(ten?.price ?? null, 2, 7), { displayValue: fmt(ten?.price ?? null, "%"), dataType: ten ? "DIRECT" : "UNAVAILABLE", source: sourceName(ten?.source), asOf: ten?.asOf ?? null }),
  metric("yield_curve", "2Y/10Y spread", curve, curve === null ? null : scale(-curve, 0, 1.5), { displayValue: fmt(curve, "%"), dataType: curve === null ? "UNAVAILABLE" : "KAIRO_CALCULATED", source: "KAIRO calculated", detail: curveState }),
...
function macroMetrics(groups: MacroObservation[][], highImpactEvents: number | null): RiskMetric[] { const all = groups.flat(); const observations = (indicator: MacroObservation["indicator"]) => all.filter((item) => item.indicator === indicator && item.value !== null).sort((a, b) => b.date.localeCompare(a.date)); const latest = (indicator: MacroObservation["indicator"]) => observations(indicator)[0]?.value ?? null; const unemployment = observations("EMPLOYMENT"); const unemploymentChange = unemployment.length > 1 ? unemployment[0]!.value! - unemployment[1]!.value! : null; return [
  metric("macro_inflation", "Inflation pressure", latest("INFLATION"), scale(latest("INFLATION"), 2, 8), { displayValue: fmt(latest("INFLATION"), "%"), dataType: latest("INFLATION") === null ? "UNAVAILABLE" : "DIRECT", source: "FMP / Alpha Vantage" }),
  metric("macro_gdp", "Real GDP", latest("GDP"), scale(latest("GDP"), -2, 4, true), { displayValue: fmt(latest("GDP"), "%"), dataType: latest("GDP") === null ? "UNAVAILABLE" : "DIRECT", source: "FMP / Alpha Vantage" }),
  metric("macro_employment", "Unemployment change", unemploymentChange, scale(unemploymentChange, 0, 1.5), { displayValue: fmt(unemploymentChange, " pp"), dataType: unemploymentChange === null ? "UNAVAILABLE" : "KAIRO_CALCULATED", source: "KAIRO calculated" }),
...
function newsMetrics(news: ProviderNewsItem[] | null): RiskMetric[] { if (!news) return [metric("news_sentiment", "Overall news sentiment", null, null), metric("negative_share", "Negative news share", null, null), metric("news_volume", "Risk-news volume", null, null)]; const scores = news.map((item) => item.overallSentimentScore).filter((value): value is number => value !== null); const sentiment = average(scores); const negativeShare = scores.length ? scores.filter((value) => value < -0.15).length / scores.length * 100 : null; const topics = new Set(news.flatMap((item) => item.topics.map((topic) => topic.toLowerCase()))); return [
  ...
  metric("negative_share", "Negative news share", negativeShare, scale(negativeShare, 10, 70), { displayValue: fmt(negativeShare, "%"), dataType: negativeShare === null ? "UNAVAILABLE" : "KAIRO_CALCULATED", source: "KAIRO calculated" }),
...
async function collectGlobalRisk(): Promise<GlobalRiskSnapshot> {
  ...
  const id = await persistGlobalRiskSnapshot(snapshot, 15); return { ...snapshot, id };
}
