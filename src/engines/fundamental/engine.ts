...
const aliases = {
  revenue: ["revenue", "totalRevenue"], grossProfit: ["grossProfit"], operatingIncome: ["operatingIncome"], netIncome: ["netIncome", "netIncomeCommonStockholders"], eps: ["eps", "epsDiluted"], ebitda: ["ebitda"],
...
export function analyzeFundamentals(input: {
  ...
  const currentIncome = income[0]; const previousIncome = income[1]; const currentBalance = balanceSheet[0]; const currentCashFlow = cashFlow[0]; const previousCashFlow = cashFlow[1]; const currentRatios = ratios[0];
  ...
  const assets = statementValue(currentBalance, aliases.assets); const debt = statementValue(currentBalance, aliases.debt); const netDebt = statementValue(currentBalance, aliases.netDebt) ?? (debt !== null && statementValue(currentBalance, aliases.cash) !== null ? debt - (statementValue(currentBalance, aliases.cash) as number) : null);
  ...
  const metrics: FundamentalMetricSet = {
    ...
    operatingMargin: ratioValue(currentRatios, ["operatingProfitMargin", "operatingProfitMarginTTM"]) ?? divide(statementValue(currentIncome, aliases.operatingIncome), revenue),
    ...
    trailingPe: input.summary.trailingPe, forwardPe: input.summary.forwardPe, peg: ratioValue(currentRatios, ["priceEarningsToGrowthRatio", "priceEarningsToGrowthRatioTTM"]), evToEbitda: ratioValue(currentRatios, ["enterpriseValueMultiple", "enterpriseValueMultipleTTM"]) ?? divide(input.summary.enterpriseValue, ebitda), evToRevenue: divide(input.summary.enterpriseValue, revenue), priceToSales: ratioValue(currentRatios, ["priceToSalesRatio", "priceToSalesRatioTTM"]), priceToBook: input.summary.priceToBook, priceToFreeCashFlow: input.summary.marketCap === null ? null : divide(input.summary.marketCap, freeCashFlow), earningsYield: input.summary.trailingPe && input.summary.trailingPe !== 0 ? 1 / input.summary.trailingPe : null, freeCashFlowYield: divide(freeCashFlow, input.summary.marketCap), dividendYield: input.summary.dividendYield,
  };
