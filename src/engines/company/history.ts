...
const fields = {
  revenue: ["revenue", "totalRevenue"], costOfRevenue: ["costOfRevenue"], grossProfit: ["grossProfit"], ebitda: ["ebitda"], operatingIncome: ["operatingIncome"], netIncome: ["netIncome", "netIncomeCommonStockholders"], dilutedEps: ["epsDiluted", "eps"], dilutedShares: ["weightedAverageShsOutDil", "weightedAverageSharesDiluted"], sharesOutstanding: ["sharesOutstanding"],
  cash: ["cashAndCashEquivalents", "cashAndShortTermInvestments"], shortTermInvestments: ["shortTermInvestments"], receivables: ["receivables"], inventory: ["inventory"], assets: ["totalAssets"], liabilities: ["totalLiabilities"], goodwill: ["goodwill"], intangibles: ["intangibleAssets", "goodwillAndIntangibleAssets"], propertyPlantEquipment: ["propertyPlantEquipment"], accountsPayable: ["accountsPayable"], debt: ["totalDebt", "shortTermAndLongTermDebt"], shortTermDebt: ["shortTermDebt"], longTermDebt: ["longTermDebt"], netDebt: ["netDebt"], equity: ["totalStockholdersEquity", "totalEquity"], currentAssets: ["totalCurrentAssets"], currentLiabilities: ["totalCurrentLiabilities"],
...
export function buildHistoricalPeriods(input: { income: FinancialStatement[]; balance: FinancialStatement[]; cashFlow: FinancialStatement[] }): HistoricalCompanyPeriod[] {
  ...
  return [...byDate.entries()].sort(([a], [b]) => b.localeCompare(a)).map(([, row]) => {
    ...
    return {
      ...
      revenue: value(row.income, fields.revenue), costOfRevenue: value(row.income, fields.costOfRevenue), grossProfit: value(row.income, fields.grossProfit), ebitda: value(row.income, fields.ebitda), operatingIncome: value(row.income, fields.operatingIncome), netIncome: value(row.income, fields.netIncome), dilutedEps: value(row.income, fields.dilutedEps), dilutedShares: value(row.income, fields.dilutedShares), sharesOutstanding: value(row.balance, fields.sharesOutstanding),
      cash, shortTermInvestments: value(row.balance, fields.shortTermInvestments), receivables: value(row.balance, fields.receivables), inventory: value(row.balance, fields.inventory), currentAssets, propertyPlantEquipment: value(row.balance, fields.propertyPlantEquipment), totalAssets: value(row.balance, fields.assets), goodwill: value(row.balance, fields.goodwill), intangibles: value(row.balance, fields.intangibles), totalDebt, shortTermDebt: value(row.balance, fields.shortTermDebt), longTermDebt: value(row.balance, fields.longTermDebt), netDebt: value(row.balance, fields.netDebt) ?? (totalDebt !== null && cash !== null ? totalDebt - cash : null), equity: value(row.balance, fields.equity), totalLiabilities: value(row.balance, fields.liabilities), accountsPayable: value(row.balance, fields.accountsPayable), currentLiabilities, workingCapital: currentAssets !== null && currentLiabilities !== null ? currentAssets - currentLiabilities : null,
