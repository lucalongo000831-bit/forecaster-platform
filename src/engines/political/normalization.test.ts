...
describe("political disclosure normalization", () => {
  ...
  it("includes disclosure identity in fingerprints", () => { const row = politicalTransaction(); expect(politicalTransactionFingerprint(row)).not.toBe(politicalTransactionFingerprint({ ...row, filingId: "another-filing" })); });
});
