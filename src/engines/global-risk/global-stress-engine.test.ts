...
const keys: GlobalRiskComponentKey[] = ["VOLATILITY", "CREDIT", "LIQUIDITY", "RATES", "MARKET_BREADTH", "EQUITY_STRESS", "CROSS_ASSET", "MACRO", "ENERGY", "POSITIONING", "NEWS_GEOPOLITICAL"];
...
describe("GlobalStressEngine", () => {
  ...
  it("does not treat missing data as zero", () => { const components = keys.map((key, index) => component(key, index < 2 ? 60 : null)); const result = calculateWeightedScore(components); expect(result.score).toBe(60); expect(result.completeness).toBe(26); });
