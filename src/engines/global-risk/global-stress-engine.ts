...
export class GlobalStressEngine {
  ...
  calculate(input: GlobalStressEngineInput): GlobalRiskSnapshot {
    ...
    return {
      id: null, status, score, previousStatus: input.history.previousStatus, previousScore: input.history.previousScore, change: input.history.previousScore === null ? null : round(score - input.history.previousScore), trend, systemicStress, confidence, dataCompleteness: weighted.completeness, components, riskDrivers, stabilizingFactors, escalationTriggers, deEscalationTriggers,
