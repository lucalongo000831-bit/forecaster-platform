...
import { parseJsonBody, requireAccount } from "@/lib/server/account-route";
...
export async function POST(request: Request) { const context = createRequestContext(request); try { const user = await requireAccount(request, context, true); return jsonSuccess(await createWatchlist(user.id, watchlistCreateSchema.parse(await parseJsonBody(request))), context, { status: 201, headers: { "Cache-Control": "no-store" } }); } catch (error) { return jsonFailure(error, context); } }
