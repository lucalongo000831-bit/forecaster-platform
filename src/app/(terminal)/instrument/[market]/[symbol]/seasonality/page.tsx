...
export default async function SeasonalityPage({ params }: { params: Promise<{ market: string; symbol: string }> }) {
  ...
  return <><div className="container-shell page-stack"><section>
  ...
  </section><section className="grid-3"><div className="soft-card p-6"><span className="small-label">Best historical month</span><div className="kpi positive mt-2">{data.bestMonth}</div><p className="muted mt-2">{data.positiveYearsPercent}% positive years</p></div><div className="soft-card p-6"><span className="small-label">Average return</span><div className="kpi positive mt-2">{formatPercent(data.averageReturn, true)}</div><p className="muted mt-2">Selected 20-year window</p></div><div className="soft-card p-6"><span className="small-label">Seasonal bias</span><div className="kpi mt-2">{data.bias}</div><p className="muted mt-2">Mock statistical reading</p></div></section></div><Footer/></>;
}
