...
export function DataSourceNotice({ source }: { source?: DataSource }) {
  if (!source || ["yahoo", "fmp", "alpha-vantage", "massive", "calculated"].includes(source)) return null;
  return <div className="soft-card flex items-center gap-3 p-4 text-sm"><DatabaseZap className="text-indigo-400" size={18}/><span><strong>{source === "mock" ? "Modalità demo" : "Dato non disponibile"}</strong> · {source === "mock" ? "I provider non hanno risposto; i valori visibili sono mock esplicitamente identificati." : "Questa sezione richiede un provider alternativo."}</span></div>;
}

export function DataUnavailable({ title = "Dato non disponibile", detail = "Questa informazione non è disponibile dai provider configurati." }: { title?: string; detail?: string }) {
