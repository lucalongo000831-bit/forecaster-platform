  ...
  return <div className="container-shell technical-workspace technical-workspace-v2" ref={shellRef} data-testid="technical-chart-workspace">
    ...
    <div className="technical-layout technical-layout-v2">
      ...
      <aside className="technical-sidebar">
        ...
        <section><header><div><span>COMPARE</span><strong>Rebased performance</strong></div><small>{activePanel.comparisons.length}/3</small></header><div className="technical-compare"><input aria-label="Compare symbol" placeholder="SPY" value={compareInput} onChange={(event) => setCompareInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") addComparison(); }}/><button onClick={addComparison} disabled={activePanel.comparisons.length >= 3}><Plus size={16}/></button></div><div className="technical-chips">{activePanel.comparisons.map((comparison) => <button key={comparison} onClick={() => updateWorkspacePanel(activePanel.id, { comparisons: activePanel.comparisons.filter((item) => item !== comparison) })}>{comparison} ×</button>)}</div><p>Comparisons are rebased to 0%. Identical panel requests share one in-flight operation.</p></section>
    ...
    {workspace.features.confluence && confluence && <section className="technical-summary" aria-label="Technical confluence summary"><article><span>TREND</span><strong>{confluence.trend}</strong><p>{confluence.reasons[0] ?? "Insufficient verified history."}</p></article><article><span>MOMENTUM</span><strong>{confluence.momentum}</strong><p>{confluence.reasons[1] ?? "Momentum input unavailable."}</p></article><article><span>STRUCTURE</span><strong>{confluence.status}</strong><p>{confluence.structure}</p></article><article><span>VOLUME</span><strong>{confluence.alignment} alignment</strong><p>{confluence.volume}</p></article></section>}
