...
export function MainPriceChart({ data, referenceValue, compact = false }: { data: TimePoint[]; referenceValue?: number; compact?: boolean }) {
  return <div className={`chart-wrap ${compact ? "chart-short" : "chart-tall"}`}>
...
export function DrawdownChart({ data }: { data: TimePoint[] }) {
  return <div className="chart-wrap chart-short"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}>
...
export function SeasonalityChart({ data }: { data: SeasonalityPoint[] }) {
  return <div className="chart-wrap chart-tall"><ResponsiveContainer width="100%" height="100%"><LineChart data={data}>
...
export function PatternChart({ data, referenceValue }: { data: TimePoint[]; referenceValue: number }) {
  return <div className="chart-wrap chart-tall"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data}>
...
export function OscillatorChart({ data }: { data: TimePoint[] }) {
  return <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={data}>
...
export function RevenueMixCharts({ products, byYear }: { products: RevenueProduct[]; byYear: RevenueYear[] }) {
  return <div className="grid-2">
    <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><BarChart data={byYear}><CartesianGrid stroke={grid} vertical={false}/><XAxis dataKey="year" tick={axis}/><YAxis tick={axis}/><Tooltip/><Bar dataKey="compute" fill="#f2b84b"/><Bar dataKey="data" fill="#e95f75"/><Bar dataKey="networking" fill="#6576ed"/><Bar dataKey="gaming" fill="#9c5dd5"/></BarChart></ResponsiveContainer></div>
    <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={products} dataKey="value" nameKey="name" outerRadius="75%" label>{products.map((item)=><Cell key={item.name} fill={item.color}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></div>
  </div>;
}
...
export function AllocationChart({ positions }: { positions: PortfolioPosition[] }) {
  ...
  return <div className="chart-wrap chart-short"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} innerRadius="48%" outerRadius="75%" dataKey="value" nameKey="name" label>{data.map((item,index)=><Cell key={item.name} fill={colors[index]}/>)}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></div>;
}

export function MarketGauge({ mood }: { mood: string }) {
  return <div className="relative mx-auto h-[260px] w-full max-w-[460px] overflow-hidden">
